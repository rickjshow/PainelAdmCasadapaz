...
class BannersGaleriaController extends Controller
{
        ...
        try {
            ...
            if ($banner) {
                ...
                if ($request->hasFile('banner_principal')) {
                    ...
                    $data['banner_principal'] = $request->file('banner_principal')->store('banners-galeria');
                }
                ...
                if ($request->hasFile('banner_principal_mobile')) {
                    ...
                    $data['banner_principal_mobile'] = $request->file('banner_principal_mobile')->store('banners-galeria');
                }
                ...
                if ($data['banner_principal'] || $data['banner_principal_mobile']) {
                    BannerGaleria::create($data);
                } else {
                    return redirect()->route('galeria.index')->with('error', 'Por favor, envie pelo menos um banner.');
                }
            }

            return redirect()->route('galeria.index')->with('success', 'Banner salvo com sucesso!');
        } catch (\Exception $e) {
            Log::error('Erro ao salvar banner: ' . $e->getMessage());
            return redirect()->route('galeria.index')->with('error', 'Erro ao salvar o banner.');
        }
    }
