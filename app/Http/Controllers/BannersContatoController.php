...
class BannersContatoController extends Controller
{
        ...
        try {
            ...
            if ($banner) {
                ...
                if ($request->hasFile('banner_principal')) {
                    ...
                    $data['banner_principal'] = $request->file('banner_principal')->store('banners-contato');
                }
                ...
                if ($request->hasFile('banner_principal_mobile')) {
                    ...
                    $data['banner_principal_mobile'] = $request->file('banner_principal_mobile')->store('banners-contato');
                }
                ...
                if ($data['banner_principal'] || $data['banner_principal_mobile']) {
                    BannerContato::create($data);
                } else {
                    return redirect()->route('contato.index')->with('error', 'Por favor, envie pelo menos um banner.');
                }
            }

            return redirect()->route('contato.index')->with('success', 'Banner salvo com sucesso!');
        } catch (\Exception $e) {
            Log::error('Erro ao salvar banner: ' . $e->getMessage());
            return redirect()->route('contato.index')->with('error', 'Erro ao salvar o banner.');
        }
    }
